...
def create_options(
    ...
    # Allow overriding the claude binary via env var (useful when bundled binary
    # has auth issues but the system-installed binary works fine)
...
_AGENT_TIMEOUT_SECONDS = 300  # 5 minutes — bundled binary can be slow to start
...
async def _run_agent(
    ...
    """Run an agent query with a timeout, capturing only the final assistant text.

    Wraps the inner coroutine with asyncio.wait_for so a hung SDK call (e.g.
    "Not logged in" binary that doesn't exit) never blocks the bot forever.
    ...
    try:
        return await asyncio.wait_for(_inner(), timeout=_AGENT_TIMEOUT_SECONDS)
    except asyncio.TimeoutError:
        logger.error("Agent SDK timed out after %ds", _AGENT_TIMEOUT_SECONDS)
        return WorkerResult(
            result_text=f"Agent timed out after {_AGENT_TIMEOUT_SECONDS}s",
            cost_usd=0, duration_ms=0, num_turns=0,
            session_id=None, is_error=True,
        )
