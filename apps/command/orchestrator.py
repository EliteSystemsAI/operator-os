...
from . import memory_manager, supabase_memory
from .worker import TASK_FAILED_SESSION_ID, run_general_agent, run_general_prime
...
# Maps slash commands → natural language prompts sent to the CC agent.
# Commands with arguments use {args} placeholder or append them inline.

_SLASH_COMMANDS: dict[str, str] = {
    "/emails":          "Check my Gmail inbox. List unread emails, summarize anything urgent, and flag anything that needs a reply today.",
    "/leads":           "Show me the GHL pipeline snapshot — qualified leads, what's stalled, and what action is needed.",
    "/revenue":         "Show me the Stripe revenue snapshot — MTD, month-on-month trend, and run rate.",
    "/morning":         "Give me the daily morning briefing — trending AI news and the top 3 script opportunities for today.",
    "/trending":        "Do a deep trend analysis across the 8 AI sub-categories and return scores and script angles.",
    "/reel":            "Analyze the latest Instagram post performance and save the results to the database.",
    "/schedule":        "Show the 14-day content calendar and flag any gaps or scheduling issues.",
    "/resource-script": "Find a free AI resource and build a 'comment X for this' viral script around it.",
    "/ads":             "Show me the Meta ads report — active campaigns, spend, performance, and any recommendations.",
    "/post":            "Help me create and format a post, add it to the content queue, and schedule it via GHL.",
    "/script":          "Write a full reel script",
    "/hooks":           "Generate 5 hook A/B variations with angle analysis for this topic",
    "/repurpose":       "Repurpose this video into 5 platform-native posts (IG, LinkedIn, TikTok, X, YouTube) and schedule via GHL",
    "/notify":          "Send a Telegram notification with this message",
    "/carousel":        "Create an Instagram carousel for",
    "/handoff":         "Capture the current session state for a fresh Claude session to resume. Write a handoff summary to tasks/todo.md.",
    "/remember":        "Append to memory/decisions.md with today's date (AEST) and this note",
    "/idea":            "Add a new content idea to data/content_ideas.json. Read the file first. Append a new entry with fields: topic (from the message), pillar (infer from: Tool Spotlights / System Reveals / Myth Busting / Results & Proof / How-To), funnel (infer TOFU/MOFU/BOFU), source='telegram', date=today (AEST), status='pending'. Write the file back. Confirm what was added. The idea topic is",
    "/backlog":         "Read data/content_ideas.json and data/content_queue.json. Show all pending ideas grouped by pillar with their funnel stage, then show current queue status (how many scheduled, pending, approved).",
    "/clients":         "Read data/client_health_snapshot.json. Summarise: when it was last updated, total open pipeline opportunities, how many are at-risk (stalled 7+ days), and list the at-risk ones by name, stage, and days stalled.",
    "/savings":         "Query Supabase time_savings table. Show total hours saved across all clients, breakdown by client, and top systems by hours saved. Format as a clean Telegram message.",
    "/clickup":         "Show ClickUp project status. Use the ClickUp API (token in CLICKUP_API_TOKEN env) to fetch tasks from list 901609565565 (Agency PM > Projects). Show open tasks grouped by status. Format as clean Telegram message.",
}


def _translate_slash_command(text: str) -> str:
    """Strip @botname suffix and translate slash commands to natural language.

    Telegram appends @botname to slash commands in groups (e.g. /emails@eliteclaudebot).
    We strip that suffix, then map the command to a natural language prompt so the
    CC agent receives clear instructions rather than raw slash commands it may not recognise.
    """
    if not text.startswith("/"):
        return text

    # Strip @botname suffix (e.g. /emails@eliteclaudebot → /emails)
    ...
    if cmd not in _SLASH_COMMANDS:
        return text  # Unknown command — pass through unchanged

    base = _SLASH_COMMANDS[cmd]
    if args:
        return f"{base}: {args}"
    return base
...
async def _save_session_summary(messages: list[dict], session_id: str) -> None:
    """Ask claude-haiku to summarise recent conversation turns and save to Supabase.

    Called every 10 turns per session. Uses the last 20 messages for the
    summary to keep token usage low. Saves via supabase_memory.save_session_summary().
    ...
    if not api_key:
        agent_log.warning("_save_session_summary: ANTHROPIC_API_KEY not set, skipping")
        return
        ...
        response = client.messages.create(
            ...
            messages=[{
                ...
                "content": (
                    ...
                    "Focus on: decisions made, actions taken, and anything pending.\n\n"
                    f"{history_text}"
                ),
            }],
        )
        summary = response.content[0].text
        supabase_memory.save_session_summary(summary, workflow="chat")
        agent_log.info("Session summary saved for session_id=%s", session_id)
    except Exception as e:
        agent_log.warning("Session summary generation failed for %s: %s", session_id, e)
...
class Orchestrator:
    ...
    async def handle_general_message(
        ...
        # Translate slash commands to natural language prompts
        text = _translate_slash_command(text)
    ...
    async def _handle_cc_agent_message(
        ...
        # Inject conversation history on first message of a fresh session
        if session.total_turns == 0:
            recent = memory_manager.get_recent_context(
                self.config.workspace_dir, topic_key
            )
            if recent:
                effective_text = recent + "\n\n" + effective_text
        ...
        if result.session_id == TASK_FAILED_SESSION_ID:
            # Agent SDK failed on this task — delete session so the next message
            # re-primes fresh with Claude Code instead of staying on OpenAI forever.
            agent_log.warning("Task failed and fell back to direct API — deleting session for re-prime")
            self.sessions.delete(topic_key)
        ...
        # Log this turn to persistent memory (fire-and-forget, never blocks)
        memory_manager.log_turn(
        ...
        # Log turn to Supabase short-term memory
        # Use the Telegram chat_id as a stable session identifier across bot restarts
        tg_session_id = str(message.chat.id)
        supabase_memory.log_message(tg_session_id, "user", message.text or text or "")
        supabase_memory.log_message(tg_session_id, "assistant", result.result_text)

        # Every 10 turns, generate and save a session summary (mid-term memory)
        recent = supabase_memory.get_recent_messages(tg_session_id, limit=100)
        if len(recent) % 10 == 0 and len(recent) > 0:
            asyncio.create_task(_save_session_summary(recent, tg_session_id))
